...
Compiler.prototype = {
    ...
    valueToString: function(val) {
        if (val.type == STRING) {
            ...
            if (val.type == I1 || val.type == I8 || val.type == I16 || val.type == I32 || val.type == CHAR) {
            ...
            if (node.operand.nick == 'Binary') {
                ...
                if (node.operand.op == T.SliceOp) {
                    if (operandType instanceof ClassType) {
                        ...
                        throw new MoyaError("Illegal slice", node.loc);
                    }
                } else {
                    var indexType = this.inferNode(node.operand.right);
                    if (operandType instanceof ClassType) {
                        var func = this.matchMethodCall(operandType, op, [indexType], [])
                        if (func) {
                            return func.returnType;
                        } else {
                            throw new MoyaError("Operator not supported", node.loc);
        ...
        if (node.op == T.DeleteOp) {
            ...
            if (node.op == T.NegativeOp) {
                if (operand.type instanceof NumberType) {
                    ...
                    var func = this.matchMethodCall(operand.type, '-neg', [], [])
                    if (func) {
                        var ret = llvm.compileCall(func.native, [operand.value]);
                        return expr(func.returnType, ret);
                    } else {
                        throw new MoyaError("Operator not supported", node.loc);
                    }
                ...
                if (operand.type instanceof NumberType || operand.type instanceof PointerType) {
                    ...
                    var func = this.matchMethodCall(operand.type, '!', [], [])
                    if (func) {
                        var ret = llvm.compileCall(func.native, [operand.value]);
                        return expr(func.returnType, ret);
                    } else {
                ...
                if (operand.type instanceof ClassType) {
                    var func = this.matchMethodCall(operand.type, 'in', [], [])
                    if (func) {
                        var ret = llvm.compileCall(func.native, [operand.value]);
                        return expr(func.returnType, ret);
                    } else {
