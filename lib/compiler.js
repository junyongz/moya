...
Compiler.prototype = {
    ...
    compileExpression: function(nodes, where) {
        var result = null;

        if (where) {
            where.forEach(function(node) {
                if (node.nick == "Assignment" && node.op == ops.Eq) {
                    node.compileAsStatement(this);
                } else {
    ...
    compileVariableAssign: function(name, rhs, loc) {
        var ownerScope = this.scope.hasVariableName(name) || this.scope;
        return ownerScope.storeVariableValue(this, name, rhs, loc);
    },
