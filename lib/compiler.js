...
Compiler.prototype = {
    ...
    castReturns: function(newType) {
        for (var i = 0, l = this.returns.length; i < l; ++i) {
            var ret = this.returns[i];
            var cast = this.valueToType(ret.expr, newType);
            ret.expr = cast;
        }
    },
    ...
    evaluateType: function(typeNode) {
        ...
        if (typeNode.nick == 'TypeId') {
            ...
            if (type) {
                if (type.pointers) {
                    type = this.getPointerType(type, typeNode.pointers);
                }
                    
                if (typeNode.optionals) {
                    type = this.getOptionalType(type, typeNode.optionals);
                }
    ...
    compileClassInit: function(classType, classScope) {
        ...
        var tableVar = this.builder.gep(self, [this.int(0), this.int(0)], null, POINTER);
        ...
        for (var i = 0, l = props.length; i < l; ++i) {
            ...
            var variable = this.builder.gep(self, [this.int(0), offset], null, prop.type);
    ...
    callFunction: function(name, args, argSymbols) {
        ...
        if (func) {
            var castArgs = this.completeCallArgs(func, argTypes, args);
    ...
    callMethod: function(object, name, args, argSymbols) {
        ...
        if (func) {
            ...
            castArgs = this.completeCallArgs(func, argTypes, castArgs);
                        
            var tableVar = this.builder.gep(object, [this.int(0), this.int(0)], null, POINTER);
            var table = this.builder.loadVariable(tableVar, "table");
            var methodVar = this.builder.gep(table, [this.builder.methodOffset(func)], null, POINTER);
    ...
    completeCallArgs: function(func, argTypes, args) {
        var castArgs = [];
        ...
        for (var i = 0, l = args.length; i < l; ++i) {
            ...
            if (arg.type instanceof OptionalType) {
                ...
                if (defaultFunc) {
                    var passed = this.compileTest(arg, 0, ops.NotEquals);
                    castArgs[i] = this.compileSingleChoice(passed, function() {
                        return this.valueToType(arg, expectedType);
                    }.bind(this), function() {
                        return this.builder.call(defaultFunc, []);
                    }.bind(this));
                } else {
                    castArgs[i] = this.valueToType(arg, expectedType);
                }
            } else {
                castArgs[i] = this.valueToType(arg, expectedType);
            }
        ...
        for (var i = argTypes.length, l = func.argTypes.length; i < l; ++i) {
            ...
            if (defaultFunc) {
                ...
                castArgs[i] = this.valueToType(defaultValue, expectedType);
            } else if (expectedType instanceof OptionalType) {
                castArgs[i] = this.builder.optional(expectedType);
            }
        }
    ...
    createFunction: function(key, func, ownerType, argSymbols, argTypes, argDefaults) {
        ...
        this.castReturns(returnType);
        instFunc.returnType = returnType || VOID;
        
        if (!didReturn) {
            if (instFunc.returnType && instFunc.returnType != VOID) {
                var optional = instFunc.returnType.withOptionals(1);
                instFunc.returnType = optional;
                this.builder.return(this.builder.optional(optional));
                this.castReturns(optional);
            } else {
    ...
    valueToType: function(value, expectedType, castObjects) {
        ...
        } else if (expectedType instanceof ClassType) {
            throw new MoyaError("Class type conversion NYI");
        } else if (expectedType instanceof PointerType) {
            throw new MoyaError("Pointer type converstion NYI");
        } else if (expectedType instanceof OptionalType) {
            if (value.type instanceof OptionalType) {
                return value;
            } else {
                return this.valueToOptional(value, expectedType);
            }
        } else if (type == STRING) {
   ...
   valueToNumber: function(val, type) {
        if (val.type == type) {
            ...
            return this.builder.numCast(val, type);
        } else if (val.type instanceof OptionalType) {
            var val = this.builder.extractValue(val, 0, val.type.type, 'opt');
            return this.valueToNumber(val, type);
        } else {
            throw new MoyaError("Illegal cast");
        }
    },
    ...
    valueToString: function(val, node) {
        if (!val.type || val.type == VOID) {
            ...
            } else if (val.type == I1 || val.type == I8 || val.type == I16 || val.type == I32) {
        ...
        } else if (val.type instanceof ClassType) {
            // XXXjoe We should be calling toString method on object
            return this.builder.string(val.type.toString());
        } else if (val.type instanceof PointerType) {
            throw new MoyaError("Pointer to string NYI", node.loc);
        } else if (val.type instanceof OptionalType) {
            var passed = this.compileTest(val, 0, ops.NotEquals);
            return this.compileSingleChoice(passed,
            function(compiler) {
                var extracted = compiler.builder.extractValue(val, 0, val.type.type, 'opt');
                return compiler.valueToString(extracted);
            }, function(compiler) {
                return compiler.builder.string('unknown');
            });
            throw new MoyaError("Optional to string NYI", node.loc);
        } else {
            throw new MoyaError("Invalid types for operation", node.loc);
        }
    },

    valueToOptional: function(val, optionalType) {
        var cast = this.valueToType(val, optionalType.type);
        return this.builder.optional(cast, optionalType);
    },
    ...
    compileVariableDeclare: function(name, type, rhs) {
        var cast = this.valueToType(rhs, type);
        return this.scope.storeVariable(name, cast, this.builder);
    },
    ...
    compileTest: function(condition, n, op) {
        ...
        if (condition.type == I1 || condition.type == I8 || condition.type == I16
            ...
            var ret = this.callOpOverride(ops.Not, condition, [], []);
            if (ret) {
            ...
            var flag = this.builder.extractValue(condition, 1, I8, 'flag');
            return this.compileTest(flag, n, op);
        } else {
            throw new MoyaError("Invalid type for truth test", condition.loc);
        }
    },

    compileChoice: function(nextCondition, nextPassed, nextFailed, node) {
        ...
        while (true) {
            ...
            if (!resultType) {
                ...
                throw new MoyaError("Different types in expression", node ? node.loc : null);
            }
    ...
    compileSingleChoice: function(condition, passed, failed, node) {
        var called = false;
