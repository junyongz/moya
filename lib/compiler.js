...
Compiler.prototype = {
    ...
    global: function(name, value) {
        ...
        if (!gv) {
            gv = this.builder.global(name, value.type, value.type.defaultValue(this.builder));
    ...
    declareExternals: function() {
        ...
        this.moyaAllocException = this.createCFunction('__cxa_allocate_exception', POINTER, [I32]);
        this.moyaBeginCatch = this.createCFunction('moyaBeginCatch', POINTER, [POINTER]);
        this.moyaEndCatch = this.createCFunction('__cxa_end_catch', VOID, []);
        this.moyaThrow = this.createCFunction('__cxa_throw', null, [POINTER, POINTER, POINTER]);
        this.moyaThrowDest = this.createCFunction('moyaThrowDestructor', VOID, [POINTER]);
        
        this.moyaCreateTypeInfo = this.createCFunction('moyaCreateTypeInfo', POINTER, [STRING]);
    ...
    callFunction: function(name, args, argSymbols, loc) {
        ...
        if (func) {
            ...
            return this.call(func, castArgs, null, loc);
        }
    },
    ...
    callMethod: function(object, name, args, argSymbols, loc) {
        ...
        if (func) {
            ...
            return this.call(castMethod, castArgs, func.type.returnType, loc);
        }
    },

    call: function(func, args, returnType, loc) {
        if (func.throws || func == this.moyaThrow) {
            this.catchFrame.markThrows(func.throws);
            ...
            var ret = this.builder.invoke(func, args, returnType, contBlock, unwindBlock, loc);
            this.builder.insert(ret);
            ...
            return this.builder.call(func, args, returnType, loc);
        }
    },
    ...
    matchCall: function(func, ownerType, argTypes, argSymbols) {
        ...
        for (var i = 0, l = argTypes.length; i < l; ++i) {
            ...
            if (declArg.type) {
                ...
                funcScope.expandType(declArg.type, symbol);
            }
        }
        ...
        for (var i = argTypes.length, l = declArgs.length; i < l; ++i) {
            ...
            if (defaultFunc) {
                if (declArg.type) {
                    ...
                    funcScope.expandType(declArg.type, expectedSymbol);
                } else {
    ...
    compileCall: function(name, object, argNodes, symbolNodes, node) {
        ...
        var ret = object ? this.callMethod(object, name, args, argSymbols, node.loc) : null;
        if (ret) {
            return ret;
        } else {
            var ret = this.callFunction(name, args, argSymbols, node.loc);
            if (ret) {
                return ret;
            } else {
...
CatchFrame.prototype = {
    ...
    matchThrow: function(type) {
        for (var typeKey in this.throwMap) {
            ...
            if (type.isTypeOrSubclass(throwType)) {
