...
var scopes = require('./scope'),
    ...
    ClosureScope = scopes.ClosureScope;
...
Compiler.prototype = {
    ...
    popScope: function(shouldNotCleanup) {
        var oldScope = this.scope;
        if (!shouldNotCleanup) {
            oldScope.cleanup(this);
        }

        this.scope = oldScope.previous;
        return oldScope;
    },
    ...
    getUnwindBlock: function(loc) {
        ...
        if (catchAll) {
            if (catchAll.name) {
                var throws = catchFrame.matchAll();
                ...
                this.pushScope();
                var didReturn = compileBody(this, catchAll.body, true);
                this.popScope(didReturn);
        ...
        function compileCatch(compiler, isCatchAll, name, type, body, loc) {
            ...
            var didReturn = compileBody(compiler, body, isCatchAll);
        ...
        function compileBody(compiler, body, isCatchAll) {
    ...
    // Not using this yet but I'll probably want to have unwinding terminate in some situations.
    ...
    log: function(value) {
        ...
        if (typeof(value) == "string") {
            str = this.builder.string(value, 'LOG');
        } else {
    ...
    compileProgram: function(moduleName, sourcePath, source, exeOutPath) {
        ...
        try {
            ...
            if (this.inspectMode == "ast") {
                ...
                this.builder.classes.forEach(function(cls) {
    ...
    compileModuleInit: function(mod) {
        ...
        for (var i = 0, l = mod.props.length; i < l; ++i) {
            ...
            if (prop.block instanceof syntax.CFunctionSyntax) {
                this.declareCFunction(prop.block, mod, prop.name, prop.accessMode, false);
            } else {
                ...
                this.scope.updateVariable(this, prop.name, mod, this.builder, rhs, loc);
            }
        }
    ...
    inheritMethods: function(classType) {
        ...
        if (baseType) {
            ...
            for (var i = 0, l = baseType.methods ? baseType.methods.length : 0; i < l; ++i) {
    ...
    linkBuiltinTypes: function(module) {
        for (var className in module.classes) {
            if (className in builtinTypes) {
                var builtinType = builtinTypes[className];
                var builtinClass = module.classes[className][0];
                builtinType.class = builtinClass;

                if (builtinClass.base) {
                    builtinType.base = builtinClass.base.compileType(this);
                }
                
                var key = utils.keyForClass(builtinClass, []);
                this.typeCache[key] = builtinType;
            }
        }
    },
    ...
    declareCFunction: function(node, mod, alias, accessMode, isLocal) {
        var args = [];
        if (node.args) {
        ...
        if (!isLocal) {
            mod.declareFunction(func, alias);
        }
    ...
    declareCType: function(node) {
        if (node.name == 'void') {
            return new p.parseTypeId(node.loc, 'Void', node.pointers);
        } else if (node.name == 'bool') {
            return new p.parseTypeId(node.loc, 'Bool', node.pointers);
        } else if (node.name == 'short') {
            return new p.parseTypeId(node.loc, 'Int16', node.pointers);
        } else if (node.name == 'int' || node.name == 'long') {
            return new p.parseTypeId(node.loc, 'Int32', node.pointers);
        } else if (node.name == 'long long' || node.name == 'long long int') {
            return new p.parseTypeId(node.loc, 'Int64', node.pointers);
        } else if (node.name == 'float') {
            return new p.parseTypeId(node.loc, 'Float', node.pointers);
        } else if (node.name == 'double') {
            return new p.parseTypeId(node.loc, 'Double', node.pointers);
        } else if (node.name == 'size_t') {
            return new p.parseTypeId(node.loc, 'Int32', node.pointers);
        } else if (node.name == 'time_t') {
            return new p.parseTypeId(node.loc, 'Int64', node.pointers);
    ...
    matchType: function(genericClass, argSymbols) {
    ...
    compileClassInit: function(classType, classScope) {
        ...
        if (classType.base) {
            var initFunc = classType.base.initFunc;
            if (initFunc) {
                var castSelf = this.builder.bitCast(self, classType.base, loc);
                var ret = this.builder.call(initFunc, [castSelf], null, loc);
    ...
    callFunction: function(name, args, argSymbols, loc) {
        ...
        if (func) {
            ...
            return this.call(func, castArgs, func.returnType, null, loc);
        }
    },
    ...
    matchFunctionCall: function(name, argTypes, argSymbols) {
        ...
        var result = oldScope.lookupFunction(name, function(genericFunc) {
            ...
            var instFunc = this.matchCall(genericFunc, null, null, argTypes, argSymbols);
            this.popScope();
            if (genericFunc.isConstructor) {
                this.popScope();
            }
    ...
    matchMethodCall: function(classType, name, argTypes, argSymbols) {
        ...
        var result = classType.lookupMethod(name, function(genericFunc, ownerType) {
            ...
            this.pushScope(new FunctionScope());
            var instFunc = this.matchCall(genericFunc, ownerType, null, argTypes, argSymbols);
            this.popScope();
            this.popScope();
    ...
    matchClosureCall: function(closureType, argTypes) {
        this.save();
                    
        this.scope = new ClosureScope(closureType);
        this.pushScope(new FunctionScope());
        var instFunc = this.matchCall(closureType.genericFunc, null, closureType, argTypes, []);
        this.popScope();
    ...
    matchCall: function(func, ownerType, closureType, argTypes, argSymbols) {
        var funcScope = this.funcScope;
        ...
        return this.getFunction(func, ownerType, closureType, funcSymbols, argTypes, argDefaults);
    },
    
    // =============================================================================================
    
    getFunction: function(func, ownerType, closureType, argSymbols, argTypes, argDefaults) {
        var key = ownerType
        ...
        if (instFunc) {
            ...
            if (func.isCFunction) {
                ...
                return this.createFunction(key, func, ownerType, closureType, argSymbols, argTypes,
                                           argDefaults);
            }
        }
    },
    
    createFunction: function(key, func, ownerType, closureType, argSymbols, argTypes, argDefaults) {
        ...
        if (func.isConstructor) {
            var type = this.matchType(func.class, argSymbols);
            instFunc.classType = type;
            instFunc.returnType = type;
            explicitReturn = true;

            var classScope = funcScope.previous;
            classScope.setClass(type);
            
            castSelf = type.construct(this.builder, func.loc);
            classScope.self = castSelf;
            this.builder.insert(castSelf);
                
            if (type.initFunc) {
                var called = this.builder.call(type.initFunc, [castSelf], null, func.loc);
        ...
        if (closureType) {
            var argStub = this.builder.stub(closureType);
            this.builder.insert(argStub);
            this.scope.previous.closureStub = argStub;
            this.scope.previous.closureFunction = instFunc;
            argStubs.push(argStub);
            argNames.push('__closure__');
            instFunc.argTypes.unshift(closureType);
            instFunc.argDefaults.unshift(null);
        }
    ...
    createClosure: function(genericFunc) {
        var type = new ClosureType(genericFunc, this.scope);
        this.scope.evaluateAll();
        return this.builder.closure(type, genericFunc.loc);
    },
    ...
    compileStatements: function(nodes) {
        ...
        for (var i = 0, l = nodes ? nodes.length : 0; i < l; ++i) {
            var didReturn = nodes[i].compileAsStatement(this);
            if (didReturn) {
                break;
            }
        }
    ...
    compileVariableAssign: function(name, rhs, loc) {
        var accessModule = this.scope.rootModule;
        var value = this.scope.updateVariable(this, name, accessModule, this.builder, rhs, loc);
        if (value) {
            return value;
        } else {
    ...
    compileCall: function(name, object, argNodes, symbolNodes, loc) {
        ...
        if (!object) {
            ...
            if (local) {
                if (symbolNodes) {
                    throw new MoyaError("Symbols not supported in this call", loc);
                }
                return local.type.call(this, local, args, loc);
            }
        }
    ...
    compileChoice: function(nextCondition, nextPassed, nextFailed, loc) {
        ...
        if (elseFailed) {
            ...
            var result = resultType.defaultValue(this.builder, loc);
            exprs.push(result);
            blocks.push(elseBlock);
        }
...
CatchFrame.prototype = {
    ...
    markThrow: function(type) {
        var key = type+'';
        if (!this.throwMap[key]) {
            this.throwMap[key] = type;
        }
        
        if (this.unwindCaught && !this.unwindCaught[key]) {
            this.invalidate();
        }
    ...
    markCatch: function(key) {
        if (!this.unwindCaught) {
            this.unwindCaught = {};
        }
        this.didCatch = true;
        this.unwindCaught[key] = true;
    },
