...
Compiler.prototype = {
    ...
    compileBinary: function(op, lhs, rhs, node) {
        if (lhs.type instanceof ClassType) {
            var ret = this.callMethod(lhs, node.op.token, [rhs], []);
            if (ret) {
                if (node.op instanceof ops.CompareOperator && ret.type != BOOL) {
                    throw new MoyaError("Function returned non-boolean type", node.loc);
                }
                
                return ret;
            } else if (node.op == ops.Equals) {
                return this.builder.compare(ops.Equals, lhs, rhs);
            } else if (node.op == ops.NotEquals) {
                var ret = this.callMethod(lhs, '==', [rhs], []);
                if (ret) {
                    return this.compileTest(ret, 0);
                } else {
                    return this.builder.compare(ops.NotEquals, lhs, rhs);
                }
            } else if (node.op == ops.NotIn) {
            ...
            } else if (op == ops.Pow || op == ops.PowEq) {
                return this.compilePow(lhs, rhs, node);
            } else if (op instanceof ops.ConcatOperator) {
                return this.compileConcat(lhs, rhs, node);
            } else if (op instanceof ops.MathOperator) {
                return this.compileBinaryMath(op, lhs, rhs, node);
            } else if (op instanceof ops.CompareOperator) {
    ...
    compileStringMath: function(op, lhs, rhs, node) {
        if (op == ops.Add || op == ops.AddEq || op == ops.Concat || op == ops.ConcatEq) {
            return this.compileConcat(lhs, rhs, node);
        } else {
            throw new MoyaError("Illegal types for operation", node.loc);
        }
    ...
    Assignment: function(node, isStatement) {
        ...
        } else if (node.op instanceof ops.MathIncrementOperator) {
            return this.compileIncrement(node, rhs);
        } else if (node.op instanceof ops.ConcatIncrementOperator) {
            return this.compileIncrement(node, rhs);
        } else {
        ...
        if (node.op == ops.Delete) {
            if (node.operand.nick == 'Binary') {
                ...
                if (operand.type instanceof ClassType) {
                    ...
                    } else if (node.operand.op == ops.Lookup) {
                        var index = this.compileNode(node.operand.right);
                        var ret = this.callMethod(operand, '-=.[]', [index], [])
                        if (ret) {
                            return ret;
                    ...
                    } else if (node.operand.op == ops.Slice) {
