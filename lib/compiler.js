...
Compiler.prototype = {
    ...
    castReturns: function(newType) {
        for (var i = 0, l = this.returns.length; i < l; ++i) {
            ...
            var cast = newType.valueToType(ret.expr, this);
            ret.expr = cast;
        }
    },
    ...
    compileClassInit: function(classType, classScope) {
        ...
        for (var i = 0, l = props.length; i < l; ++i) {
            ...
            if (prop.type) {
                rhs = prop.type.valueToType(rhs, this);
            } else {
                prop.type = rhs.type;
            }
    ...
    completeCallArgs: function(func, argTypes, args) {
        ...
        for (var i = 0, l = args.length; i < l; ++i) {
            ...
            if (arg.type instanceof OptionalType) {
                ...
                if (defaultFunc) {
                    var passed = this.compileTruthTest(arg);
                    castArgs[i] = this.compileSingleChoice(passed, function() {
                        return expectedType.valueToType(arg, this);
                    }.bind(this), function() {
                        return this.builder.call(defaultFunc, []);
                    }.bind(this));
                } else {
                    castArgs[i] = expectedType.valueToType(arg, this);
                }
            } else {
                castArgs[i] = expectedType.valueToType(arg, this);
            }
        ...
        for (var i = argTypes.length, l = func.argTypes.length; i < l; ++i) {
            ...
            if (defaultFunc) {
                ...
                castArgs[i] = expectedType.valueToType(defaultValue, this);
    ...
    compileVariableDeclare: function(name, type, rhs) {
        var cast = type.valueToType(rhs, this);
    ...
    compileTruthTest: function(condition) {
        return this.compileTest(condition, 0, ops.NotEquals);
    },
