...
Compiler.prototype = {
    ...
    declareClass: function(classDecl) {
        ...
        for (var i = 0, l = nodes.length; i < l; ++i) {
            ...
            if (node.nick == 'FunctionDecl') {
                if (node.id.nick == 'TypeId' && node.id.id == 'This') {
                    this.declareConstructor(node, cls);
        ...
        if (!hasConstructors) {
            var emptyFunction = T.parseEmptyFuncDecl(cls.loc);
            this.declareConstructor(emptyFunction, cls);
        }
    ...
    declareConstructor: function(fnDecl, genericClass) {
        ...
        if (minimumArgCount == -1) {
            minimumArgCount = argItems.length;
        }
    ...
    precompileClass: function(genericClass, inputTypes) {
        ...
        for (var i = 0, l = nodes.length; i < l; ++i) {
            ...
            if (node.nick == 'Property') {
                if (node.type) {
                    var prop = classType.addProperty(node.name);
                    prop.type = scope.evaluateType(node.type);
                    propertyMap[node.name] = node.body;
                } else {
                    var prop = classType.addProperty(node.name);
                    propertyMap[node.name] = node.body;
                }
    ...
    compileCall: function(name, argNodes, inputTypeNodes, node) {
        var inputTypes = [];
        if (inputTypeNodes) {
            for (var i = 0, l = inputTypeNodes.length; i < l; ++i) {
    ...
    Call: function(node, isStatement) {
        ...
        if (callable.nick == "Identifier") {
            return this.compileCall(callable.id, node.args, inputTypeNodes, node);
        } else if (callable.nick == "TypeId") {
            return this.compileCall(callable.id, node.args, inputTypeNodes, node);
        } else {
            throw new MoyaError('Call type not yet implemented', node.loc);
