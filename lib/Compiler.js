...
Compiler.prototype = {
    compileModule: function(name, ast, debugMode) {
        ...
        this.printInt = this.bridge.declareFunction('printInt', Void.code, [I32.code], ['n']);
    ...
    Binary: function(node) {
        ...
        if (node.op == T.AddOp) {
            if (lhs.type == rhs.type) {
                return expr(lhs.type, this.bridge.compileAdd(lhs.value, rhs.value));
            } else if (1/*both are numbers*/) {
                var cast = this.bridge.castNumber(rhs.value, lhs.type.code);
                return expr(lhs.type, this.bridge.compileAdd(lhs.value, cast));
            } else {
                throw Error("Incompatible");
            }
        } else if (node.op == T.SubtractOp) {
            return expr(lhs.type, this.bridge.compileSubtract(lhs.value, rhs.value));
        } else if (node.op == T.MultiplyOp) {
            return expr(lhs.type, this.bridge.compileMultiply(lhs.value, rhs.value));
        } else if (node.op == T.DivideOp) {
            return expr(lhs.type, this.bridge.compileDivide(lhs.value, rhs.value));
        }
    },
