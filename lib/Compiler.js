...
var VTABLEPOINTER = new SequenceType('i8');
...
function GenericFunction(name) {
    ...
    this.realFunctionMap = [];
    this.class = null;
}

GenericFunction.prototype = {
    keyWithArgs: function(argTypes, inputTypes) {
        var key = '';
    ...
    match: function(argTypes, inputTypes, scope) {
        ...
        var key = this.keyWithArgs(argTypes, inputTypes);
        var realFunc = this.realFunctionMap[key];
        if (realFunc) {
            return realFunc;
        }
        ...
        for (var i = argTypes.length, l = args.length; i < l; ++i) {
            ...
            if (arg.type && arg.defaultValue) {
                var expectedType = scope.compiler.inferNode(arg.defaultValue);
                funcScope.expandType(arg.type, expectedType);
            }
        }
        ...
        var returnType = null;
        if (this.returns) {
            returnType = funcScope.evaluateType(this.returns);
        } else if (this.ast.body) {
            scope.compiler.pushScope(funcScope);
            var argItems = this.ast.args.items;
            for (var i = 0, l = argItems.length; i < l; ++i) {
                var argItem = argItems[i];
                var argType = argTypes[i];
                funcScope.storeVariableType(argItem.innerName, argType);
            }
            
            scope.compiler.inferBlock(this.ast.body);
            scope.compiler.popScope();
            
            returnType = funcScope.returnType;
        }
        
        var func = new RealFunction(this.name, funcScope.typeLocals, argTypes, returnType);
        func.generic = this;
        this.realFunctionMap[key] = func;
        return func;
    }
}
...
Scope.prototype = {
    ...
    matchFunction: function(name, argTypes, inputTypes) {
        if (this.previous) {
            return this.previous.matchFunction(name, argTypes, inputTypes);
        }
    },
};
    ...
    matchFunction: function(name, argTypes, inputTypes) {
        ...
        if (funcs) {
            for (var i = 0, l = funcs.length; i < l; ++i) {
                var func = funcs[i];
                var realFunc = func.match(argTypes, inputTypes, this);
                if (realFunc) {
                    return realFunc;
                }
            }
...
FunctionScope.prototype = fool.subclass(Scope, {
    evaluateType: function(typeNode) {
        if (typeNode.nick == "TypeId") {
            ...
            var cls = this.previous.lookupClass(typeNode.id, function(cls) {
    ...
    matchFunction: function(name, argTypes, inputTypes) {
        ...
        for (var i = 0, l = classInputNames.length; i < l; ++i) {
            ...
            if (inputTypeName == name) {
                ...
                return this.previous.matchFunction(c.name, argTypes, inputTypes);
            }
        }
...
Compiler.prototype = {
    ...
    precompileClass: function(genericClass, inputTypes) {
        ...
        for (var i = 0, l = genericClass.props.length; i < l; ++i) {
            ...
            if (genericProp.type) {
                ...
                prop.type = scope.evaluateType(genericProp.type);
        ...
        for (var propertyName in classType.properties) {
            ...
            classScope.lookupVariableType(propertyName, genericClass.ast);
    ...
    matchFunction: function(name, argTypes, inputTypes) {
        var realFunc = this.scope.matchFunction(name, argTypes, inputTypes);
        if (realFunc) {
            if (!realFunc.native) {
    ...
    matchMethod: function(classType, name, argTypes, inputTypes) {
        if (!classType) {
            return null;
        }
        ...
        var classScope = new ClassScope(classType);
        this.pushScope(classScope);
        var realFunc = classType.matchMethod(name, argTypes, inputTypes, classScope);
        this.popScope();
    ...
    compileCall: function(name, object, argNodes, inputTypeNodes, node) {
        ...
        var func = object ? this.matchMethod(object.type, name, argTypes, inputTypes) : null;
        if (func) {
            ...
            var func = this.matchFunction(name, argTypes, inputTypes);
            if (func) {
                if (argTypes.length < func.argTypes.length) {
                    for (var i = argTypes.length, l = func.argTypes.length; i < l; ++i) {
                        var defaultCall = func.argDefaults[i-argTypes.length];
    ...
    inferCall: function(node, isStatement) {
        ...
        if (callable.nick == "Identifier") {
            var thiss = this.scope.getThis();
            var func = this.matchMethod(thiss ? thiss.type : null, callable.id, argTypes, inputTypes)
            if (func) {
            ...
            var func = this.matchMethod(lhsType, callable.right, argTypes, inputTypes)
            if (func) {
                return func.returnType;
            } else {
