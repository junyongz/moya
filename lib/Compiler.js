...
Compiler.prototype = {
    ...
    compileClass: function(classDecl) {
        var name = classDecl.name.id;
        var type = new ObjectType(name);
        this.classMap[name] = type;
        ...
        for (var i = 0, l = nodes.length; i < l; ++i) {
            ...
            if (node.nick == "Assignment") {
                if (node.left.nick == "Identifier") {
                    offsets[node.left.id] = size;
                    // Compile initializer
                    // Get back type
                    // Add type size to total size
                } else {
    ...
    compileFunction: function(fnDecl) {
        ...
        if (fnDecl.args) {
            ...
            for (var i = 0, l = argItems.length; i < l; ++i) {
                var argItem = argItems[i];
                this.scope.store(argItem.innerName, expr(sigArgs[i], funcAndArgs[i]));
            }
        }
    ...
    Call: function(node, isStatement) {
        ...
        if (callable.nick == "Identifier") {
            ...
            if (cls) {
                return expr(cls, this.bridge.compileCall(this.newObject, [this.getInt(4)]));
            } else {
    ...
    While: function(node, isStatement) {
        var testBlock = this.bridge.createBlock('test', this.activeFunc);
        var loopBlock = this.bridge.createBlock('loop', this.activeFunc);
        var afterBlock = this.bridge.createBlock('after', this.activeFunc);
