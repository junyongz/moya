...
GenericFunction.prototype = {
    match: function(argTypes, inputTypes, scope) {
        ...
        var remainingTypeArgs = this.inputTypes.length;
        ...
        for (var i = 0, l = argTypes.length; i < l; ++i) {
            ...
            var arg = this.args[i];
            var expectedType = funcScope.evaluateType(arg.type, argType);
            if (!expectedType || !argType.isTypeOrSubclass(expectedType)) {
                return null;
            }
        }
        
        if (remainingTypeArgs > 0) {
            return null;
        } else {
            var returnType = null;
            if (this.returns) {
                if (this.returns.typeVarName) {
                    var typeIndex = typeArgMap[arg.typeVarName];
                    returnType = typeArgList[typeIndex];
                } else {
                    returnType = this.returns.type;
                }
            }
    ...
    declareFunction: function(fnDecl) {
        ...
        if (fnDecl.args) {
            ...
            for (var i = 0, l = argItems.length; i < l; ++i) {
                ...
                var arg = {name: argItem.innerName, type: argItem.type};
                func.args.push(arg);
            }
        }
