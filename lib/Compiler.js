...
Compiler.prototype = {
    ...
    printSourceError: function(message, loc, sourcePath, source) {
        ...
        if (loc) {
            ...
            var carets = '^'.repeat(loc.last_column - loc.first_column);
    ...
    compileProgram: function(moduleName, sourcePath, source, debugMode) {
        if (!source) {
            if (!sourcePath) {
                if (moduleName) {
                    sourcePath = this.findModule(moduleName);
                } else {
        ...
        try {
            ...
            if (debugMode == "ast") {
                ...
                this.compileModule(moduleName, sourcePath, ast);
                this.compileMain();
                llvm.endModule(debugMode == 'ir' ? 1 : 0);
                llvm.executeMain();
            }
        } catch (exc) {
            if (exc.hash) {
                this.printSourceError('Syntax error', exc.hash.loc, sourcePath, source);
            } else if (exc.message) {
                this.printSourceError(exc.message, exc.loc, sourcePath, source);
            } else {
                throw exc;
            }
            // throw exc;
    ...
    declareExternals: function() {
        ...
        this.newObject = llvm.declareExternalFunction('newObject', STRING.native, [I32.native]);
    },

    compileModule: function(moduleName, sourcePath, ast) {
        ...
        for (var i = 0, l = nodes.length; i < l; ++i) {
            ...
            if (node.nick == "FunctionDecl") {
                ...
                this.collectImportSet(node, imports);
        ...
        mod = new Module(moduleName, sourcePath);
        ...
        this.pushScope(new ModuleScope(mod));
    ...
    compileImport: function(moduleName, sourcePath) {
        var source = fs.readFileSync(sourcePath, 'utf8').trim();
        var ast = parser.parse(source);
        
        return this.compileModule(moduleName, sourcePath, ast);
    },

    findModule: function(modulePath) {
        var fullPath = path.resolve(modulePath);
    ...
    collectImportSet: function(importNode, imports) {
        var names = importNode.moduleNames;
        for (var i = 0, l = names.length; i < l; ++i) {
            this.collectImportNames(names[i], imports);
        }
    },
    
    collectImportNames: function(nameSet, imports) {
        var paths = [];
    ...
    compileCType: function(node) {
        if (node.name == 'void') {
            ...
            if (node.pointers == 0) {
                return new T.parseTypeId(node.loc, 'Int8');
            } else if (node.pointers == 1) {
                return new T.parseTypeId(node.loc, 'String');
            } else {
                return new T.parseTypeId(node.loc, 'Pointer');
            }
        } else {
            return new T.parseTypeId(node.loc, 'Pointer');
        }
    ...
    precompileClass: function(genericClass, argSymbols) {
        ...
        classType.size = llvm.setStructBody(classType.nativeStruct, structTypes);
    ...
    compileFunction: function(realFunc, ownerType, argVariables) {
        ...
        if (realFunc.classType) {
            ...
            var raw = llvm.compileCall(this.newObject, [this.getInt(classType.size+ftableSize)]);
    ...
    valueToString: function(val) {
        if (val.type == STRING) {
            ...
            return llvm.declareString(val.type.name);
        }
    },
    ...
    getTypeDefault: function(type) {
        if (type.isNumber) {
            if (type == F32) {
                ...
                return expr(type, llvm.compileInteger(type.size, 0));
            }
        } else {
    ...
    compileTest: function(condition, n) {
        if (condition.type == I1 || condition.type == I8 || condition.type == I16
            ...
            var zero = llvm.compileInteger(condition.type.size, n);
