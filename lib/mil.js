...
exports.Global = function(name, type, value) {
    this.name = name;
    this.type = type;
    this.value = value;
}
...
exports.Global.prototype = fool.subclass(Expression, {
    serialize: function(writer) {
        if (this.value) {
            return writer.write('GLOBAL ' + this.name + ' = ' + this.value);
        } else {
            return writer.write('GLOBAL ' + this.name);
        }
    },
    ...
    generate: function(builder) {
        ...
        if (this.value) {
            return llvm.getGlobal(type, this.name, this.value.compile(builder));
        } else {
            return llvm.getGlobal(type, this.name, null);
        }
    },
});
