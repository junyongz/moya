...
Node.prototype = {
    ...
    valueAsConstant: function(loc) {
        throw new MoyaError("Expected constant", loc);
    },
...
exports.Function = function(func, argSymbols, argTypes, argDefaults) {
    this.name = func.name;
    this.func = func;
...
exports.Function.prototype = fool.subclass(Expression, {
    serialize: function(writer) {
        writer.begin();
    ...
    generate: function(builder) {
        ...
        var returnTypeNative = returnType ? returnType.compile(builder) : VOID.compile(builder);
    ...
    addToClosure: function(localVar) {
        if (localVar.closureIndex == -1) {
            if (!this.closureVars) {
                this.closureVars = [localVar];
                this.closureType = new StructType('closure');
            } else {
                this.closureVars.push(localVar);
            }

            localVar.closureIndex = this.closureVars.length-1;
        }
    },
    
    compileClosure: function(builder) {
        if (this.closure) {
            return this.closure;
        } else if (this.closureVars) {
            this.closureType.fieldTypes = this.closureVars.map(function(localVar) {
                localVar.type.compile(builder);
                return localVar.type;
            }.bind(this));

            this.closureType.compile(builder);
            
            var closurePtrType = this.closureType.withPointers(1);
            var size = this.closureType.objectSize;
                        
            if (size) {
                var tempBlock = builder.tempBlock();
                builder.setInsertBlock(tempBlock);

                var closureRaw = builder.call(builder.newObject, [builder.int(size)]);
                var closureCast = builder.bitCast(closureRaw, closurePtrType);
                builder.insert(closureCast);
                this.closure = closureCast;

                this.closureVars.forEach(function(localVar) {
                    localVar.closure = closureCast;
                });

                this.blocks[0].prepend(tempBlock);
            } else {
                this.closure = builder.null(POINTER);
            }
            return this.closure;
        } else {
            return null;
...
exports.Closure = function(type) {
    this.type = type;
}

exports.Closure.prototype = fool.subclass(Expression, {
    serialize: function(writer) {
        var closureScope = this.type.parentScope;
        var closure = closureScope.func.closure;
        if (closure){
            return closure.write(writer);
        } else {
            return writer.temp('CLOSURE ' + this.type);
        }
    },
            
    generate: function(builder) {
        var closureScope = this.type.parentScope;
        var closure = closureScope.func.compileClosure(builder);
        if (closure) {
            return closure.compile(builder);
        } else {
            return builder.null(POINTER).compile(builder);
        }
    },
});
...
exports.Block.prototype = fool.subclass(Node, {
    ...
    prepend: function(block) {
        for (var i = 0, l = block.statements.length; i < l; ++i) {
...
exports.Stub.prototype = fool.subclass(Expression, {
    serialize: function(writer) {
        if (this.closure) {
            var closure = this.closure.write(writer);
            var ptr = writer.temp("GEP " + closure + "[0, " + this.closureIndex + "]", "ptr");
            writer.write("STORE " + ptr + " = " + 'STUB (' + this.type + ')');
        }
    ...
    generate: function(builder) {
        if (this.closure) {
            var closure = this.closure.compile(builder);
            var offsets = [llvm.compileInteger(32, 0), llvm.compileInteger(32, this.closureIndex)];
            var ptr = llvm.getPointer(closure, offsets);
            llvm.storeVariable(ptr, this.value);
        }
        return this.value;
    },
});
...
exports.String = function(string, name) {
    this.string = string;
    this.name = name;
    this.type = STRING;
}
...
exports.String.prototype = fool.subclass(Expression, {
    ...
    generate: function(builder) {
        var name = this.name || this.string.replace(/[^a-zA-Z]/g, '');
        return llvm.declareString(this.string, 'STR' + name);
    },
});

// *************************************************************************************************

exports.ConstStruct = function(type, fields) {
    this.type = type;
    this.fields = fields;
}

exports.ConstStruct.prototype = fool.subclass(Expression, {
    serialize: function(writer) {
        var fields = this.fields.map(function(field) { return field.write(writer); });
        return writer.temp('STRUCT ' + fields.join(', '));
    },
...
exports.Tuple = function(type, fields) {
    this.type = type;
    this.fields = fields;
}

exports.Tuple.prototype = fool.subclass(Expression, {
    serialize: function(writer) {
        var fields = this.fields.map(function(field) { return field.write(writer); });
        return writer.temp('TUPLE ' + fields.join(', '));
    },

    generate: function(builder) {
        var t = this.type.compile(builder);
        var structVar = llvm.createVariable('tuple', t);
        var struct = llvm.loadVariable(structVar, 'tuplevar');
        
        this.fields.forEach(function(field, i) {
            var value = field.compile(builder);
            struct = llvm.insertValue(struct, i, value);
        }.bind(this));
...
exports.Optional.prototype = fool.subclass(Expression, {
    ...
    generate: function(builder) {
        ...
        if (this.hasValue) {
            ...
            var value = this.type.type.defaultValue(builder, this.loc).compile(builder);
            struct = llvm.insertValue(struct, 0, value);
        }
...
exports.CreateVariable.prototype = fool.subclass(Expression, {
    serialize: function(writer) {
        if (this.closure) {
            var closure = this.closure.write(writer);
            writer.write('@' + this.name + ' = GET FROM CLOSURE ' + closure + '[' + this.closureIndex + '] (' + this.type + ')');
        } else {
            writer.write('@' + this.name + ' = ALLOC (' + this.type + ')');
        }
        return '@' + this.name;
    },
    ...
    generate: function(builder) {
        ...
        if (this.closure) {
            var closure = this.closure.compile(builder);
            var offsets = [llvm.compileInteger(32, 0), llvm.compileInteger(32, this.closureIndex)];
            return llvm.getPointer(closure, offsets);
        } else {
