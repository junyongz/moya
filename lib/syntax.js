    ...
    compile: function(compiler, isStatement) {
        ...
        if (lhs.type instanceof ClassType) {
            ...
            if (prop) {
                ...
                var variable = compiler.builder.gep(lhs, [compiler.int(0), offset], null, prop.type);
    ...
    compile: function(compiler, isStatement) {
        if (isStatement) {
            ...
            if (type instanceof OptionalType) {
                optional = compiler.builder.optional(type);
            } else {
                type = compiler.getOptionalType(type, 1);
                optional = compiler.builder.optional(type);
            }
            return compiler.compileVariableDeclare(this.name, type, optional);
        } else {
    ...
    compileIfExpression: function(compiler) {
        ...
        function conditions(compiler) {
            if (i < pairs.length) {
                ...
                return compiler.compileTest(clause, 0, ops.NotEquals);
            }
        }
        ...
        return compiler.compileChoice(conditions, passes, fails);
    },
});
