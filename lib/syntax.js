...
Syntax.prototype = {
    ...
    compileStatement: function(compiler) {
    ...
    compileAsBlock: function(compiler) {
        return this.compileStatement(compiler);
    },

    compileAsFunction: function(compiler) {
        compiler.markReturned(true);

        var ret = this.compile(compiler);
        var returned = compiler.builder.return(ret, this.loc);
...
function BlockSyntax(body, where, isImperative) {
    this.body = body;
    this.where = where;
...
BlockSyntax.prototype = fool.subclass(Syntax, {
    ...
    declareInModule: function(compiler, module) {
        var funcNode = new exports.FunctionSyntax(new exports.IdentifierSyntax('@main'));
    ...
    compile: function(compiler) {
        ...
        return compiler.compileExpression(this.body, this.where);
    },

    compileStatement: function(compiler) {
        var expr = compiler.compileExpression(this.body, this.where);
        compiler.builder.insert(expr);
        return false;
    },

    compileAsBlock: function(compiler) {
        return compiler.compileStatements(this.body);
    },
    ...
    compileAsFunction: function(compiler) {
        if (this.isImperative) {
            ...
            var ret = compiler.compileExpression(this.body, this.where);
            var returned = compiler.builder.return(ret, this.loc);
            compiler.addReturn(returned);
...
TupleSyntax.prototype = fool.subclass(Syntax, {
    ...
    compile: function(compiler) {
        throw new Syntax("Tuples NYI", this.loc);
    },
...
IdentifierSyntax.prototype = fool.subclass(Syntax, {
    ...
    compileAssign: function(compiler, assignNode, rhs) {
        return compiler.compileVariableAssign(this.name, rhs, assignNode.loc);
    },
...
PrintSyntax.prototype = fool.subclass(Syntax, {
    ...
    compileStatement: function(compiler) {
        ...
        var ret = compiler.builder.call(compiler.printString, [asString], null, this.loc);
        compiler.builder.insert(ret);
        return false;
    },
});
...
TypeAssignmentSyntax.prototype = fool.subclass(Syntax, {
    ...
    compileAssign: function(compiler, assignNode, rhs) {
        var type = this.type.compileType(compiler);
        return compiler.compileVariableDeclare(this.name, type, rhs, assignNode.loc);
    },
});
...
ThrowSyntax.prototype = fool.subclass(Syntax, {
    ...
    compileStatement: function(compiler) {
        ...
        var excBuf = compiler.call(compiler.moyaAllocException, [excSize], null, null, this.loc);
        ...
        var ret = compiler.call(compiler.moyaThrow, args, null, null, this.loc);
        compiler.builder.insert(ret);
    ...
    matchCatchers: function(compiler, catchers, catchFrame) {
        ...
        for (var i = 0, l = catchers.length; i < l; ++i) {
            ...
            if (!catcher.decl) {
                ...
                catchAll = {name: null, type: null, body: catcher.body, loc: catcher.loc};
            } else if (catcher.decl.nick == "Identifier") {
                if (catchAll) {
                    throw new MoyaError("Multiple catch-alls", catcher.loc);
                }

                catchAll = {name: catcher.decl.name, type: null, body: catcher.body,
                            loc: catcher.loc};
            } else if (catcher.decl.nick == "TypeAssignment") {
                var type = catcher.decl.type.compileType(compiler);
                var key = type+'';
                if (key in matches) {
                ...
                if (catchFrame.matchThrow(type)) {
                    matches[key] = {name: catcher.decl.name, type: type, body: catcher.body,
                                    loc: catcher.loc};
                } else {
