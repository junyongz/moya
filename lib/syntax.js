...
ThrowSyntax.prototype = fool.subclass(Syntax, {
    ...
    compile: function(compiler, isStatement) {
        ...
        if (exc.type == F32 || exc.type == F64) {
            throw new MoyaError('Illegal to throw floating point numbers', this.loc);
        }
            
        var excCast = exc.valueToType(POINTER, compiler, this.loc);
        var typeInfo = exc.type.typeInfo(compiler.builder);
        var args = [excCast, compiler.builder.bitCast(typeInfo, POINTER)];
...
TrySyntax.prototype = fool.subclass(Syntax, {
    ...
    compile: function(compiler, isStatement) {
        ...
        var exc = compiler.builder.extractValue(landingPad, 0, POINTER, 'exc');
        ...
        function compileCatch(name, type, body, loc) {
            ...
            var excVar = compiler.builder.call(compiler.moyaBeginCatch, [exc]);
            var excCast = compiler.builder.bitCast(excVar, type);
            compiler.scope.storeVariable(name, excCast, compiler.builder, loc);
            
            var ti = type.typeInfo(compiler.builder);
