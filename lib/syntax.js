...
function FunctionSyntax(id, args, returns, schedule) {
    this.id = id;
...
IntegerSyntax.prototype = fool.subclass(NumberSyntax, {
    ...
    compile: function(compiler, isStatement) {
        if (this.unit == 'i1') {
            return compiler.builder.int(this.value, 1);
        } else if (this.unit == 'i8') {
            return compiler.builder.int(this.value, 8);
        } else if (this.unit == 'i16') {
            return compiler.builder.int(this.value, 16);
        } else if (this.unit == 'i32') {
            return compiler.builder.int(this.value, 32);
        } else if (this.unit == 'i64') {
            return compiler.builder.int(this.value, 64);
        } else if (this.unit == 'f') {
            return compiler.builder.float32(this.value);
        } else if (this.unit == 'd') {
            return compiler.builder.float64(this.value);
        } else if (this.unit == 'c') {
            return compiler.builder.int(this.value, 8);
        } else if (this.unit) {
...
StringSyntax.prototype = fool.subclass(Syntax, {
    ...
    compile: function(compiler, isStatement) {
        return compiler.builder.string(this.string);
    },
});
...
ListSyntax.prototype = fool.subclass(Syntax, {
    ...
    compile: function(compiler, isStatement) {
        ...
        var listObj = compiler.builder.call(listCons, []);

        var addFunc = compiler.matchMethodCall(listType, "add", [commonType], []);
        for (var i = 0, l = itemValues.length; i < l; ++i) {
            var item = itemValues[i];
            var cast = compiler.valueToType(item, commonType);
            var ret = compiler.builder.call(addFunc, [listObj, cast]);
...
IdentifierSyntax.prototype = fool.subclass(Syntax, {
    ...
    compile: function(compiler, isStatement) {
        if (this.id == 'true') {
            return compiler.builder.true();
        } else if (this.id == 'false') {
            return compiler.builder.false();
        } else if (this.id == 'compiler') {
            ...
            var result = compiler.scope.lookupVariableValue(this.id, compiler.builder);
            if (result) {
                return result;
            } else {
    ...
    compile: function(compiler, isStatement) {
        ...
        if (lhs.type instanceof ClassType) {
            ...
            if (prop) {
                ...
                var variable = compiler.builder.gep(lhs, [compiler.int(0), offset], prop.type);
                return compiler.builder.loadVariable(variable, this.right);
            } else {
...
CastSyntax.prototype = fool.subclass(Syntax, {
    ...
    compile: function(compiler, isStatement) {
        ...
        return compiler.valueToType(value, type, true);
    },
});
...
PrintSyntax.prototype = fool.subclass(Syntax, {
    ...
    compile: function(compiler, isStatement) {
        ...
        var asString = compiler.valueToString(printed, this.expr);
        return compiler.builder.call(compiler.printString, [asString]);
    },
});
...
CallSyntax.prototype = fool.subclass(Syntax, {
    ...
    compile: function(compiler, isStatement) {
        ...
        if (callable.nick == "Identifier") {
            return compiler.compileCall(callable.id, null, this.args, symbolNodes, this);
...
ReturnSyntax.prototype = fool.subclass(Syntax, {
    ...
    compile: function(compiler, isStatement) {
        ...
        var value = this.expr.compile(compiler);
        var returned = compiler.builder.return(value);
...
IfSyntax.prototype = fool.subclass(Syntax, {
    ...
    compileIfBlock: function(compiler) {
        ...
        for (var i = 0, l = pairs.length; i < l; ++i) {
            ...
            var eq = compiler.compileTest(condition, 1);
            
            var ifBlock = compiler.builder.block('then');
            var elseBlock = compiler.builder.block('else');

            compiler.builder.conditionalJump(eq, ifBlock, elseBlock);
    ...
    compileIfExpression: function(compiler) {
        ...
        var afterBlock = compiler.builder.block('result');
        var elseBlock;
        var resultType;
        var exprs = [];
        var blocks = [];
        for (var i = 0, l = pairs.length; i < l; ++i) {
            ...
            var ifBlock = compiler.builder.block('then');
            elseBlock = compiler.builder.block('else');

            compiler.builder.conditionalJump(eq, ifBlock, elseBlock);
            compiler.builder.setInsertBlock(ifBlock);
            var result = pair.block.compile(compiler);
            if (!resultType) {
        ...
        if (this.else) {
            var result = this.else.compile(compiler);
            exprs.push(result);
            blocks.push(elseBlock);
        } else {
            var result = compiler.getTypeDefault(resultType);
            exprs.push(result);
            blocks.push(elseBlock);
        }
        ...
        return compiler.builder.phi(resultType, exprs, blocks);
    },
});
...
WhileSyntax.prototype = fool.subclass(Syntax, {
    ...
    compile: function(compiler, isStatement) {
        ...
        compiler.builder.jump(testBlock);

        compiler.builder.setInsertBlock(testBlock);
        var condition = this.clause.compile(compiler);
        var eq = compiler.compileTest(condition, 1);
        compiler.builder.conditionalJump(eq, loopBlock, afterBlock);
