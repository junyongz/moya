...
Syntax.prototype = {
    ...
    compileCall: function(compiler, symbolNodes, argNodes, loc) {
        var callable = this.compile(compiler);
        var args = argNodes.map(function(arg) { return arg.expr.compile(compiler); });
        return callable.type.call(compiler, callable, args, loc);
    },
};
...
AnonFuncSyntax.prototype = fool.subclass(Syntax, {
    ...
    compileAsExpression: function(compiler) {
        var genericFunc = compiler.declareFunction(this);
        genericFunc.module = compiler.scope.rootModule;
        return compiler.createClosure(genericFunc);
    },
});
...
CFunctionSyntax.prototype = fool.subclass(Syntax, {
    ...
    declareInModule: function(compiler, module) {
        compiler.declareCFunction(this, module, null, PrivateAccess, false);
    },
    
    compileAsExpression: function(compiler) {
        var mod = compiler.scope.rootModule;
        var cfunc = compiler.declareCFunction(this, mod, null, PrivateAccess, true);
        var argTypes = cfunc.args.map(function(arg) { return arg.type.compileType(compiler); });
        var func = compiler.getFunction(cfunc, null, null, [], argTypes, []);
...
function ImplementsSyntax(left, right) {
    this.left = left;
    this.right = right;
}
exports.ImplementsSyntax = ImplementsSyntax;

ImplementsSyntax.prototype = fool.subclass(Syntax, {
    nick: "Implements",

    write: function(writer) {
        writer.begin('Implements');
...
IntegerSyntax.prototype = fool.subclass(NumberSyntax, {
    ...
    declareAlias: function(compiler, module, accessMode) {
...
ListSyntax.prototype = fool.subclass(Syntax, {
    ...
    compileAsExpression: function(compiler) {
        ...
        var listType = compiler.matchType(listClass, [symbol]);
...
TypeIdSyntax.prototype = fool.subclass(Syntax, {
    ...
    declareAlias: function(compiler, module, accessMode) {
        return module.declareTypeAlias(this.name, null, accessMode);
    },
    ...
    compileType: function(compiler) {
        ...
        var type = symbol.matchArgs(0, function(genericClass, argSymbols) {
            return compiler.matchType(genericClass, argSymbols);
        });
        if (type) {
...
TypeArgumentsSyntax.prototype = fool.subclass(Syntax, {
    ...
    expandType: function(scope, argSymbol) {
        ...
        return scope.previous.lookupClass(className, function(genericClass) {
            argSymbol.matchType(genericClass, this.args.length-1,
...
function TypeDefSyntax(access, name, base) {
    this.accessMode = access;
    this.name = name;
    this.base = base;
}
exports.TypeDefSyntax = TypeDefSyntax;

TypeDefSyntax.prototype = fool.subclass(Syntax, {
    nick: "TypeDef",

    write: function(writer) {
        writer.begin('TypeDef');
        if (this.accessMode) {
            var access = this.accessMode == PrivateAccess ? 'private' :'public';
            writer.attr('access', access);
        }
        this.name.write(writer);
        if (this.base) {
            writer.begin('base');
    ...
    declareInModule: function(compiler, module) {
        this.declareAlias(compiler, module, this.accessMode);
    },
    
    declareAlias: function(compiler, module, accessMode) {
        var base = this.base.declareAlias(compiler, module, accessMode);
        return module.declareTypeAlias(this.name.name, base, accessMode);
    },

    compileType: function(compiler) {
        return this.base.compileType(compiler);
    },
});
...
function YieldSyntax(expr, isYieldAll) {
    this.expr = expr;
    this.isYieldAll = isYieldAll;
}
exports.YieldSyntax = YieldSyntax;

YieldSyntax.prototype = fool.subclass(Syntax, {
    nick: "Yield",

    write: function(writer) {
        writer.begin('Yield', {});
        if (this.isYieldAll) {
            writer.attr('all', 'true');
        }
        if (this.expr) {
            this.expr.write(writer);
        }
...
function ReadSyntax(expr) {
    this.expr = expr;
}
exports.ReadSyntax = ReadSyntax;

ReadSyntax.prototype = fool.subclass(Syntax, {
    nick: "Read",
...
function WriteSyntax(left, right, isWriteAll) {
    this.left = left;
    this.right = right;
    this.isWriteAll = isWriteAll;
}
exports.WriteSyntax = WriteSyntax;

WriteSyntax.prototype = fool.subclass(Syntax, {
    nick: "Write",
