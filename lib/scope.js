...
FunctionScope.prototype = fool.subclass(Scope, {
    ...
    storeVariable: function(name, rhs, builder, loc) {
        ...
        if (!local) {
            local = builder.createVariable(name, rhs.type, loc);
    ...
    expandType: function(typeNode, argSymbol) {
        if (typeNode.nick == "TypeId") {
            if (typeNode.name in this.localSymbols) {
                this.localSymbols[typeNode.name] = argSymbol;
            }
        } else if (typeNode.nick == "TypeArguments") {
            var className = typeNode.args[0].name;
