...
exports.ensureTuple = function(loc, item) {
    if (item instanceof TupleSyntax) {
        return item;
    } else {
        var node = new TupleSyntax([item]);
        node.loc = loc;
        return node;
    }
}
...
exports.parseBlock = function(loc, block, where, isImperative) {
    if (!block) {
        block = [];
    ...
    if (block.length == 1 && !where && !isImperative) {
        return block[0];
    }

    var node = new BlockSyntax(block, where, isImperative);
    node.loc = loc;
    return node;
}
