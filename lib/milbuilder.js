...
MILBuilder.prototype = {
    compile: function(moduleCache) {
        llvm.beginModule("Moya", this.shouldDebug ? 1 : 0);
        this.compileCode();
        this.compileMain(moduleCache);
        llvm.endModule(this.shouldOptimize ? 1 : 0, this.inspectMode == 'ir' ? 1 : 0);
    },
    ...
    makeExecutable: function(exeOutPath) {
        ...
        var libPath = path.resolve(module.id, '..', '..', 'moyallvm', 'src', 'MoCore.cpp');
        
        var args = [libPath, objectPath, "-o", exeOutPath];
		this._runExe('clang', args).then(function() {
    		this._runExe(exeOutPath).then(function(output) {
    			console.log(output);
    		}.bind(this));
		}.bind(this));
    },
    
    _runExe: function(exePath, args) {
    ...
    compileCode: function() {
        if (this.shouldDebug) {
            ...
            for (var i = 0, l = this.modules.length; i < l; ++i) {
                ...
                var name = path.basename(mod.path);
                var dirPath = path.dirname(mod.path);
    ...
    cfunc: function(name, returnType, argTypes, loc) {
        var node = new mil.CFunction(name, returnType, argTypes);
