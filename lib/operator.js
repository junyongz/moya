    ...
    compile: function(compiler, node) {
        ...
        if (lhs.type instanceof ClassType) {
            ...
            if (lhs.type == rhs.type) {
                ...
                var cast = rhs.type.valueToType(lhs, compiler);
                return compiler.builder.compare(this, cast, rhs);
            } else {
                var cast = lhs.type.valueToType(rhs, compiler);
    ...
    combine: function(compiler, node, lhs, rhs) {
        if (lhs.type instanceof ClassType) {
            ...
            if (this == ops.Add || this == ops.AddEq) {
                var lhsCast = lhs.valueToString(compiler);
                var rhsCast = rhs.valueToString(compiler);
            ...
            if (this == ops.Add || this == ops.AddEq) {
                return compiler.builder.gep(lhs, [rhs], null, lhs.type);
            } else {
                throw new MoyaError("Illegal types for operation", node.loc);
            }
...
PowOperator.prototype = fool.subclass(BinaryOperator, {
    combine: function(compiler, node, lhs, rhs) {
        if (lhs.type instanceof ClassType) {
            ...
            var lhsCast = F64.valueToType(lhs, compiler);
            var rhsCast = F64.valueToType(rhs, compiler);
...
AssignOperator.prototype = fool.subclass(Operator, {
    ...
    assign: function(compiler, node, rhs) {
        if (node.left.nick == "Identifier") {
            return this.compileVariableAssign(compiler, node.left.name, rhs);
    ...
    compileVariableAssign: function(compiler, name, rhs) {
        ...
        if (classScope) {
            ...
            if (prop) {
                ...
                var variable = compiler.builder.gep(classScope.self, offsets, null, prop.type);

                var cast = prop.type.valueToType(rhs, compiler);
            ...
            var cast = type.valueToType(rhs, compiler);
    ...
    compilePropertyAssign: function(compiler, object, propertyName, rhs, node) {
        if (object.type instanceof ClassType) {
            ...
            if (prop) {
                ...
                var variable = compiler.builder.gep(object, [compiler.int(0), offset], null, prop.type);
