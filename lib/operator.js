...
AssignOperator.prototype = fool.subclass(Operator, {
    ...
    assign: function(compiler, node, rhs) {
        if (node.left.nick == "Identifier") {
            return this.compileVariableAssign(compiler, node.left.name, rhs, node.loc);
        } else if (node.left.nick == "TypeAssignment") {
            var type = compiler.evaluateType(node.left.type);
            return compiler.compileVariableDeclare(node.left.name, type, rhs, node.loc);
        } else if (node.left.nick == "Get") {
            var object = node.left.left.compile(compiler);
            return this.compilePropertyAssign(compiler, object, node.left.right, rhs,
                                              node.left.loc);
    ...
    compileVariableAssign: function(compiler, name, rhs, loc) {
        var classScope = compiler.scope.isProperty(name);
        if (classScope) {
            var prop = classScope.classType.getProperty(name);
            if (prop) {
                var offset = compiler.builder.propOffset(classScope.classType, prop.name, loc);
                var offsets = [compiler.int(0), offset];
                var variable = compiler.builder.gep(classScope.self, offsets, null, prop.type, loc);

                var cast = prop.type.valueToType(rhs, compiler, loc);
                compiler.builder.storeVariable(variable, cast, loc);
            }
        } else {
            var type = compiler.scope.lookupVariableType(name);
            if (!type) {
                type = rhs.type;
            }
            var cast = type.valueToType(rhs, compiler, loc);
            return compiler.scope.storeVariable(name, cast, compiler.builder, loc);
        }
    },

    compilePropertyAssign: function(compiler, object, propertyName, rhs, loc) {
        if (object.type instanceof ClassType) {
            var prop = object.type.getProperty(propertyName);
            if (prop) {
                var offset = compiler.builder.propOffset(object.type, prop.name, loc);
                var variable = compiler.builder.gep(object, [compiler.int(0), offset], null,
                                                    prop.type, loc);
