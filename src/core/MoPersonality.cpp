...
// *************************************************************************************************
// I have modified this file to use Moya types for exception handling instead of C++ std::type_info
// *************************************************************************************************
...
{
    moyaBeginCatch(unwind_exception);
    if (native_exception)
    {
                   ...
                   _Unwind_Exception* unwind_exception)
{
    ...
    return reinterpret_cast<const MoTypeInfo *>(
                   ...
                   _Unwind_Exception* unwind_exception)
{
    ...
    return (const MoTypeInfo*)readEncodedPointer(&classInfo, ttypeEncoding);
}
...
exception_spec_can_catch(int64_t specIndex, const uint8_t* classInfo,
                         uint8_t ttypeEncoding, const MoTypeInfo* excpType,
                         void* adjustedPtr, _Unwind_Exception* unwind_exception)
{
    ...
    while (true)
    {
        ...
        if (*ttypePtr == 0)
            break;
        // We can get the MoTypeInfo simply by performing a
        // R_ARM_TARGET2 relocation, and cast the result to MoTypeInfo.
        const MoTypeInfo* catchType =
            static_cast<const MoTypeInfo*>(read_target2_value(ttypePtr));
                         ...
                         void* adjustedPtr, _Unwind_Exception* unwind_exception)
{
    ...
    while (true)
    {
        ...
        if (ttypeIndex == 0)
            break;
        const MoTypeInfo* catchType = get_shim_type_info(ttypeIndex,
                                                               classInfo,
                                                               ttypeEncoding,
                                                               true,
                                                               unwind_exception);
...
static void scan_eh_tab(scan_results &results, _Unwind_Action actions,
            ...
            while (true)
            {
                ...
                if (ttypeIndex > 0)
                {
                    ...
                    else if (native_exception)
                    {
                        ...
                        const MoTypeInfo* excpType =
                            static_cast<const MoTypeInfo*>(exception_header->exceptionType);
    ...
    // XXXjoe LLVM optimizes-out my EH tables if there is no cleanup. Until
    // I find a good way to prevent that, I obviously don't want to terminate
    // in those situations
    results.reason = _URC_CONTINUE_UNWIND;
    return;
}
...
*/
 ...
 _Unwind_Reason_Code
#ifdef __USING_SJLJ_EXCEPTIONS__
__moya_personality_sj0
#else
__moya_personality_v0
#endif
...
// XXXjoe Not using this, and it uses a lot of cxxabi functions I don't want to port
#if 0
...
__cxa_call_unexpected(void* arg)
{
    ...
    catch (...)
    {
        ...
        if (native_old_exception)
        {
            ...
            {
                const MoTypeInfo* excpType =
                    static_cast<const MoTypeInfo*>(new_exception_header->exceptionType);
                adjustedPtr =
                                              ...
                                              excpType, adjustedPtr, unwind_exception))
                {
                    ...
                    moyaBeginCatch(&new_exception_header->unwindHeader);
            ...
            const MoTypeInfo* excpType =
                static_cast<const MoTypeInfo*>(&typeid(std::bad_exception));
